...
SELECT
    ...
    SUM(IIF(OD.status = 'SUCCESS', OI.quantity * OI.priceEach * (1 - OI.discount), 0)) as totalRevenue,
    SUM(IIF(OI.type = 'PRODUCT' AND OD.status = 'SUCCESS', OI.quantity * OI.priceEach * (1 - OI.discount), 0)) as productRevenue,
    SUM(IIF(OI.type = 'SERVICE' AND OD.status = 'SUCCESS', OI.quantity * OI.priceEach * (1 - OI.discount), 0)) as serviceRevenue,
